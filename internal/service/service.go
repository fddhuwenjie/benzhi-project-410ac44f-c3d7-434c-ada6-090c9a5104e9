...
import (
	...
	"github.com/benzhi/relay-survey/internal/service/keylock"
...
type Service struct {
	...
	mu           sync.RWMutex
	planMu       sync.Mutex
	caseLocks    *keylock.Pool
	requestLocks [64]sync.Mutex
}

func New(r storage.Repository) *Service {
	return &Service{repo: r, caseLocks: keylock.NewPool()}
}
...
func (s *Service) lockRequest(requestID string) func() {
	h := fnv.New32a()
	_, _ = h.Write([]byte(requestID))
	lock := &s.requestLocks[h.Sum32()%uint32(len(s.requestLocks))]
	lock.Lock()
	return lock.Unlock
}
...
func (s *Service) mutate(m Meta, id, eventType string, fn func(*casework.InterferenceCase) error, summary func(*casework.InterferenceCase) any) (*casework.InterferenceCase, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	unlockRequest := s.lockRequest(m.RequestID)
	defer unlockRequest()
	unlockCase := s.caseLocks.Reserve(id).Lock()
	defer unlockCase()

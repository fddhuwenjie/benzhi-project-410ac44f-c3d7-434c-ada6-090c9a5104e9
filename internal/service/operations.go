...
func (s *Service) EvidenceBatch(m Meta, id string, batch []casework.EvidenceRecord) (*casework.InterferenceCase, error) {
	return s.EvidenceBatchContext(context.Background(), m, id, batch)
}
func (s *Service) EvidenceBatchContext(ctx context.Context, m Meta, id string, batch []casework.EvidenceRecord) (*casework.InterferenceCase, error) {
	if err := requireRole(m.Actor, casework.RoleEngineer); err != nil {
		return nil, err
	}
	if len(batch) > 50 {
	...
	return s.mutateContext(ctx, m, id, "EVIDENCE_BATCH_ACCEPTED", func(c *casework.InterferenceCase) error { return c.AddEvidenceBatch(batch) }, func(c *casework.InterferenceCase) any {
